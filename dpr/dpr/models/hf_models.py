...
from transformers import RagModel, DPRReader
...
def get_bert_tokenizer(pretrained_cfg_name: str, do_lower_case: bool = True):
    ...
    return BertJapaneseTokenizer.from_pretrained(pretrained_cfg_name)
...
class HFBertEncoder(BertModel):
    ...
    def forward(self, input_ids: T, token_type_ids: T, attention_mask: T) -> Tuple[T, ...]:
        if self.config.output_hidden_states:
            ...
            #                                                  attention_mask=attention_mask)
            ### transformers BERT output is now BaseModelOutputWithPooling class
            bert_output = super().forward(input_ids=input_ids, token_type_ids=token_type_ids,
                                                             attention_mask=attention_mask)
